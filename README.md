...
레이아웃만은 Figma가 아니라 사전신청 페이지의 통용 문법을 따랐다 — 네비게이션 없음,
세로 1단 가운데 정렬, 폼은 첫 화면 안, 실제 스크린샷이 유일한 비주얼. 바탕을 어둡게 깐
이유는 앱이 원래 다크 UI라서다. 밝은 면에 올리면 스크린샷이 배경에 붙여 넣은 것처럼
떠 보인다.
...
`SignupForm.astro`는 **어두운 면 위에 놓이는 것을 전제로** 색을 잡았다. 밝은 면으로 옮기면
입력창·동의문·오류색이 전부 안 보이므로 그때 색부터 다시 봐야 한다. 오류색이 팔레트의
`danger-500`이 아닌 이유도 같다 — 어두운 바탕에서 대비가 안 나와 밝은 쪽으로 올렸다.
